...
export class App {
    ...
    constructor() {
        ...
        this.$window.on('keydown', this.handleKeydown);
        this.$window.on('keyup',   this.handleKeyup);

        // Handle submission of the time entry form
        this.$root.on('submit', this.selectors.entryForm, function (e) {
            e.preventDefault();
        ...
        this.$root.on('click', this.selectors.deleteRow, function (e) {
    ...
    resetElements() {
        var $notes = $(App.selectors.notes);
        this.$notes = $notes.length > 1 ? $notes.eq(1) : $notes.eq(0);
        var $startTime = $(App.selectors.startTime);
        this.$startTime = $startTime.length > 1 ? $startTime.eq(1) : $startTime.eq(0);
        var $endTime = $(App.selectors.endTime);
        this.$endTime = $endTime.length > 1 ? $endTime.eq(1) : $endTime.eq(0);
    }
    ...
    showGapWarning($row, startTime, endTime) {
    ...
    refreshTimeEntries(html, deleting) {
        var self = this;
